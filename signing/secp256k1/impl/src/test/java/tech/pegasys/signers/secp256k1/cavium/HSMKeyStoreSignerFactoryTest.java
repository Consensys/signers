...
public class HSMKeyStoreSignerFactoryTest {
  ...
  public void success()
    ...
    final KeyStore.PrivateKeyEntry entry = mock(KeyStore.PrivateKeyEntry.class);
    ...
    when(keystore.getEntry(any(), any())).thenReturn(entry);
    when(ksp.getKeyStore()).thenReturn(keystore);
    final Signer signer = (new HSMKeyStoreSignerFactory(ksp)).createSigner("0x");
