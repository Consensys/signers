...
* Not simplistic:
  ...
  - Ethereum is complex and signing utilities must handle this complexity and operate correctly and securely 
  - All signing code should align with well-established Ethereum abstractions and terminology used in Ethereum specifications 
...
The Signing utilities design prioritizes meeting current requirements in the simplest, clearest way over attempting to anticipate future functionality. As a result, the design:
...
* Uses dependency injection
  ...
  - Signing utilties will not use a dependency injection framework
...
* Consider exception and error handling as part of the overall design;  avoid checked exceptions
...
Signing utilities follow the [Google code style](https://google.github.io/styleguide/javaguide.html) and uses spotless to ensure consistency of formatting. 
...
Logging is important for understanding what the Signing utilies are doing at any given time and investigating defects.  During development, add logging to aid in these cases. 
...
* _Warn_ 
  ...
  Anything that can potentially cause application oddities but from which automatic recovery will occur

* _Error_ 

  Any error which is fatal to the operation, but not the containing application itself (for example, missing data)

* _Fatal_  

  An error that forces a shutdown of the containing application
